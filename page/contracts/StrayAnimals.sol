...
contract StrayAnimals {
    ...
    function addPet(
        ...
        petCount ++;
        pets[petCount] = Pet(petCount, _name, _breed, _age, _health, _image, msg.sender, false);
        require(bytes(_name).length > 0, "Pet name is required.");
        require(bytes(_breed).length > 0, "Pet breed is required.");
        emit PetAdded(petCount, _name, _breed, _age, _health, _image, msg.sender, false);
    }
    ...
    function getPetInfo(uint256 _petId) public view returns (string memory _name, string memory _breed, uint256 _age, string memory _health, string memory _image, address _adopter, bool _isAdopted) {
        Pet storage pet = pets[_petId];
        return (pet.name, pet.breed, pet.age, pet.health, pet.image, pet.adopter, pet.isAdopted);
    }
}
